...
import org.http4s.websocket.WebSocketFrame as Http4sWebSocketFrame
...
private[http4s] object Http4sWebSockets {
  def pipeToBody[F[_], REQ, RESP](
                                 ...
                                 )(implicit F: Temporal[F]): F[Pipe[F, Http4sWebSocketFrame, Http4sWebSocketFrame]] = {
    if ((!o.concatenateFragmentedFrames) && (!o.ignorePong) && (!o.autoPongOnPing) && o.autoPing.isEmpty) F.unit.map { _ =>
      ...
      Channel.bounded[F, Chunk[Http4sWebSocketFrame]](64).map { c =>
        (in: Stream[F, Http4sWebSocketFrame]) =>
          ...
          val ignorePingPongs = optionallyIgnorePingPong(autoPongs, ignorePing = true, o.ignorePong)
          val autoPings = o.autoPing match {
            case Some((interval, frame)) => (c.send(Chunk(frameToHttp4sFrame(frame))) >> F.sleep(interval)).foreverM[Unit]
            case None => F.unit
          }
          ...
          c.stream
            .concurrently(Stream.exec((outputProducer >> c.close.void, autoPings).asInstanceOf[(IO[Unit], IO[Unit])].parTupled.void.asInstanceOf[F[Unit]]))
